 ...
 *       "description": "내가 만든 웹앱들을 모아보는 스튜디오",
 *       "category": "포털",
 *       "thumbnail": "/thumbs/studio.png"
 *     }
 *   ]
 * }
 */
...
function PlaceholderThumb({ seed, label }: { seed: string; label: string }) {
  ...
  const bg = `hsl(${h} ${s}% ${l}%)`;
  return (
    <div className="w-full aspect-[16/9] rounded-xl flex items-center justify-center text-white/90 text-xl font-semibold shadow-inner"
         style={{ background: bg }}>
      {label}
    </div>
  );
...
export default function AppsHub() {
  ...
  useEffect(() => { document.title = "apps.huny.dev"; }, []);
...
function runDevTests() {
  const tests: Array<[string, () => boolean]> = [
    ...
    ["hashToHsl: hue range", () => { const { h } = hashToHsl("test"); return h >= 0 && h < 360; }],
  ];
  // group output
